...
const ManageAppointments = () => {
  ...
  const [isGeneratingInvoice, setIsGeneratingInvoice] = useState<number | null>(
    null,
  );
  const [isGeneratingCombinedInvoice, setIsGeneratingCombinedInvoice] =
    useState<boolean>(false);
  const [selectedAppointmentIds, setSelectedAppointmentIds] = useState<
    number[]
  >([]);
  const [isSendingWhatsapp, setIsSendingWhatsapp] = useState<number | null>(
    null,
  );
  const [isSendingCombinedWhatsapp, setIsSendingCombinedWhatsapp] =
    useState<boolean>(false);
  ...
  const [sortBy] = useState<AppointmentSortBy>(AppointmentSortBy.START_TIME);
  ...
  const fetchAllCalendarData = useCallback(async () => {
    ...
    try {
      ...
      const [
        employeesRes,
        appointmentsTableRes,
        appointmentsCalendarRes,
        availabilitiesRes,
        clientsRes,
      ] = await Promise.all([
        axios.get<Employee[]>(`${API_BASE_URL}/employees`),
        axios.get<[Appointment[], number]>(`${API_BASE_URL}/appointments`, {
          params: appointmentParamsForTable,
        }),
        axios.get<[Appointment[], number]>(`${API_BASE_URL}/appointments`, {
          params: appointmentParamsForCalendar,
        }),
        axios.get<EmployeeAvailability[]>(
  ...
  const handleGenerateInvoice = async (appointmentId: number) => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/invoices/${appointmentId}/generate`,
        {
          responseType: "blob",
        },
      );
      ...
      console.error(
        `Error al generar factura para la cita ${appointmentId}:`,
        err,
      );
      const errorMessage =
        err.response?.data?.message || "Error al generar la factura.";
  ...
  const handleGenerateCombinedInvoice = async () => {
    if (selectedAppointmentIds.length === 0) {
      setError(
        "Selecciona al menos una cita para generar una factura combinada.",
      );
      toast.warn(
        "Selecciona al menos una cita para generar una factura combinada.",
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/invoices/generate-combined`,
        {
      ...
      link.setAttribute(
        "download",
        `factura_combinada_${new Date().getTime()}.pdf`,
      );
      document.body.appendChild(link);
      link.click();
      link.parentNode?.removeChild(link);
      ...
      reader.onload = () => {
        try {
          ...
          const errorMessage =
            errorData.message ||
            "Error de validación al generar factura combinada.";
          setError(errorMessage); // Mostrar error en la tabla
          toast.error(errorMessage); // Notificación de error
        } catch (parseError) {
          const errorMessage =
            "Error de validación al generar factura combinada (formato de error desconocido).";
          setError(errorMessage); // Mostrar error en la tabla
          toast.error(errorMessage); // Notificación de error
        }
      };
  ...
  const handleDeleteAppointment = async (appointmentId: number) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Error al eliminar la cita.";
      setError(errorMessage); // Mostrar error en la tabla
      toast.error(errorMessage); // Notificación de error
    }
  };
  ...
  const handleSendInvoiceByWhatsapp = async (appointmentId: number) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/invoices/${appointmentId}/send-whatsapp`,
      );
      toast.success(
        response.data.message || "Factura enviada por WhatsApp exitosamente.",
  ...
  const handleSendCombinedInvoiceByWhatsapp = async () => {
    if (selectedAppointmentIds.length === 0) {
      setError(
        "Selecciona al menos una cita para enviar una factura combinada por WhatsApp.",
      );
      toast.warn(
        "Selecciona al menos una cita para enviar una factura combinada por WhatsApp.",
      ); // Notificación de advertencia
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/invoices/send-combined-whatsapp`,
        {
          appointmentIds: selectedAppointmentIds,
        },
      );
      toast.success(
        response.data.message ||
          "Factura combinada enviada por WhatsApp exitosamente.",
      ); // Notificación de éxito
      setSelectedAppointmentIds([]);
    } catch (err: any) {
      ...
      const errorMessage =
        err.response?.data?.message ||
        "Error al enviar la factura combinada por WhatsApp.";
      setError(errorMessage); // Mostrar error en la tabla
      toast.error(errorMessage); // Notificación de error
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="admin-content-container">
        <p>Cargando calendario y citas...</p>
      </div>
    );
  }
  ...
  return (
    <div className="admin-content-container">
      ...
      <div className="filter-controls">
        ...
        >
          <div className="form-group">
            <label htmlFor="filterClientId" className="form-label">
              Cliente:
            </label>
            <select
              id="filterClientId"
          ...
          <div className="form-group">
            <label htmlFor="filterEmployeeId" className="form-label">
              Empleado:
            </label>
            <select
              id="filterEmployeeId"
          ...
          <div className="form-group">
            <label htmlFor="filterPaymentStatus" className="form-label">
              Estado Pago:
            </label>
            <select
      ...
      <div className="button-group" style={{ marginBottom: "10px" }}>
        {" "}
        {/* Aplicamos la clase de grupo de botones */}
        <button
          className="action-button"
          style={{ backgroundColor: "var(--color-primary)" }}
          onClick={handleGenerateCombinedInvoice}
          disabled={
            selectedAppointmentIds.length === 0 || isGeneratingCombinedInvoice
          }
        >
          {isGeneratingCombinedInvoice
            ? "Generando..."
            : `Facturar Seleccionadas (${selectedAppointmentIds.length})`}
        </button>
        <button
          className="action-button"
          ...
          disabled={
            selectedAppointmentIds.length === 0 || isSendingCombinedWhatsapp
          }
        >
          {isSendingCombinedWhatsapp
            ? "Enviando..."
            : `Enviar WhatsApp (${selectedAppointmentIds.length})`}
        </button>
      </div>
          ...
          <tbody>
            {appointments.map((appointment) => {
              const canBeFactured =
                ...
                appointment.paymentStatus === PaymentStatus.PAGADO;
              ...
              return (
                <tr key={appointment.id}>
                  ...
                  <td>
                    $
                    {parseFloat(appointment.price?.toString() || "0").toFixed(
                      2,
                    ) || "0.00"}
                  </td>
                  ...
                  <td>
                    ...
                    <button
                      ...
                      disabled={
                        !canBeFactured || isGeneratingInvoice === appointment.id
                      }
                    >
                      {isGeneratingInvoice === appointment.id
                        ? "Generando..."
                        : "Facturar"}
                    </button>
                    <button
                      ...
                      onClick={() =>
                        handleSendInvoiceByWhatsapp(appointment.id)
                      }
                      disabled={
                        !canBeFactured || isSendingWhatsapp === appointment.id
                      }
                    >
                      {isSendingWhatsapp === appointment.id
                        ? "Enviando..."
                        : "Enviar WhatsApp"}
                    </button>
      ...
      <div className="pagination-controls">
        {" "}
        {/* Aplicamos la clase de paginación */}
        <button
        ...
        <span>
          Página {currentPage} de {totalPages} (Total: {totalAppointments}{" "}
          citas)
        </span>
        <button
          onClick={() =>
