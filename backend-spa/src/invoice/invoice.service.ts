...
export class InvoiceService {
  ...
  async generateInvoicePdf(appointment: Appointment): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc.on('error', (err) => {
        this.logger.error(
          `Error en PDFDocument al generar factura para cita ${appointment.id}:`,
          err,
        );
        reject(err);
      });
      ...
      try {
        ...
        const logoPath = path.join(
          __dirname,
          '../../../my-spa/src/assets/imagenes/logo-casabonita.png',
        );
        ...
        doc.text('Dirección: Cr. 50 & CL. 133 Sur, Caldas, Antioquia', {
          align: 'center',
        });
        doc.text('Teléfono: +57 321 757 1992', { align: 'center' });
        doc.text('Email: Casabonitacentroestetico@gmail.com', {
          align: 'center',
        });
        doc.moveDown();
        doc
        ...
        doc
          .moveDown()
          .fontSize(12)
          .text(`Factura No: ${appointment.id}`, { align: 'right' });
        doc.text(
          `Fecha: ${appointmentDate.toLocaleDateString('es-ES', { timeZone: 'America/Bogota' })}`,
          { align: 'right' },
        );
        ...
        doc.text(
          `Fecha y Hora: ${appointmentDate.toLocaleString('es-ES', { dateStyle: 'full', timeStyle: 'short', timeZone: 'America/Bogota' })}`,
        );
        ...
        const price = parseFloat(appointment.price?.toString() || '0').toFixed(
          2,
        );
        doc.fillColor('black').fontSize(12);
  ...
  async generateCombinedInvoicePdf(
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.on('error', (err) => {
        this.logger.error(
          `Error en PDFDocument al generar factura combinada:`,
          err,
        );
        reject(err);
      });
      ...
      try {
        ...
        doc
          .fillColor('#0c4a6e')
          .fontSize(25)
          .text('Factura de Servicios Combinada', { align: 'center' });
        doc.fillColor('black');
        doc.fontSize(10).text('Casa Bonita SPA', { align: 'center' });
        doc.text('Dirección: Cr. 50 & CL. 133 Sur, Caldas, Antioquia', {
          align: 'center',
        ...
        doc
          ...
          .fillColor('#0c4a6e')
          .fontSize(14)
          .text('Servicios Realizados:', { underline: true });
        ...
        appointments.forEach((app, index) => {
          ...
          doc
            .fontSize(12)
            .text(
              `Cita #${app.id} - ${date.toLocaleDateString('es-ES', { timeZone: 'America/Bogota' })}`,
            );
          doc.fontSize(10).text(`  Servicio: ${app.treatment?.name || 'N/A'}`);
          doc.text(`  Profesional: ${app.employee?.name || 'N/A'}`);
          doc.text(
            `  Fecha y Hora: ${date.toLocaleString('es-ES', { dateStyle: 'full', timeStyle: 'short', timeZone: 'America/Bogota' })}`,
          );
        ...
        doc
          ...
          .fontSize(16)
          .fillColor('#1f2937')
          .text(`Total Combinado a Pagar: $${total.toFixed(2)}`, {
            align: 'right',
          });
