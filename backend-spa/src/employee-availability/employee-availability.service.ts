...
export class EmployeeAvailabilityService {
  ...
  async create(
    ...
    if (
      createEmployeeAvailabilityDto.startTime >=
      createEmployeeAvailabilityDto.endTime
    ) {
    ...
    const newAvailability = this.employeeAvailabilityRepository.create(
  ...
  async findOne(id: number): Promise<EmployeeAvailability> {
    const availability = await this.employeeAvailabilityRepository.findOne({
      where: { id },
    });
    if (!availability) {
  ...
  async update(
    ...
    if (
      updateEmployeeAvailabilityDto.startTime &&
      updateEmployeeAvailabilityDto.endTime
    ) {
      if (
        updateEmployeeAvailabilityDto.startTime >=
        updateEmployeeAvailabilityDto.endTime
    ...
    if (!availability) {
      throw new NotFoundException(
        `Disponibilidad con ID ${id} no encontrada para actualizar`,
      );
    }
  ...
  async remove(id: number): Promise<void> {
    ...
    if (result.affected === 0) {
      throw new NotFoundException(
        `Disponibilidad con ID ${id} no encontrada para eliminar`,
      );
    }
  }
