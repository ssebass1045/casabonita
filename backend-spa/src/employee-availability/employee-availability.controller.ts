...
import {
  ...
  ParseIntPipe,
  ValidationPipe,
  Query, // Importa Query para parámetros de consulta
} from '@nestjs/common';
...
export class EmployeeAvailabilityController {
  ...
  create(
    ...
    return this.employeeAvailabilityService.create(
  ...
  update(
    ...
    return this.employeeAvailabilityService.update(
      id,
  ...
  findByEmployeeAndDay(
    ...
    @Param('dayOfWeek', new ValidationPipe({ transform: true }))
    dayOfWeek: DayOfWeek, // Valida el enum
  ) {
    return this.employeeAvailabilityService.findByEmployeeAndDay(
      employeeId,
      dayOfWeek,
    );
  }
