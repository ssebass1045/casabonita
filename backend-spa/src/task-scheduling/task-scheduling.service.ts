...
export class TaskSchedulingService {
  ...
  async handleAppointmentReminders() {
    ...
    this.logger.log(
      `Buscando citas para: ${tomorrowStart.toISOString()} hasta ${tomorrowEnd.toISOString()}`,
    );
    ...
    if (appointmentsToSendReminder.length === 0) {
      this.logger.log(
        'No hay recordatorios de citas para enviar en este momento.',
      );
      return;
    }

    this.logger.log(
      `Encontradas ${appointmentsToSendReminder.length} citas para enviar recordatorio.`,
    );
    ...
    for (const appointment of appointmentsToSendReminder) {
      if (appointment.client?.phone) {
        try {
          ...
          this.logger.log(
            `Recordatorio enviado para la cita ID: ${appointment.id}`,
          );
        } catch (error) {
          this.logger.error(
            `Error al enviar recordatorio para la cita ID: ${appointment.id}`,
            error,
          );
        }
      } else {
        this.logger.warn(
          `La cita ID: ${appointment.id} no tiene un número de teléfono de cliente para enviar recordatorio.`,
        );
      }

      if (appointmentsToSendReminder.length > 1) {
        // Solo añade la pausa si hay más de un mensaje que enviar
        this.logger.log(
          'Pausando por 5 segundos para respetar el límite de tasa...',
        );
        await delay(5000);
      }
    }
