...
export class AppointmentService {
  ...
  private async checkRelatedEntities(
    ...
    if (dto.treatmentId) {
      await this.treatmentService.findActiveOne(dto.treatmentId);
    }
  }
  ...
  private async validateAppointmentTime(
    ...
    const availabilities =
      await this.employeeAvailabilityService.findByEmployeeAndDay(
        employeeId,
        dayOfWeek,
      );
    ...
    const appointmentStartTimeStr = new Intl.DateTimeFormat(
      'es-CO',
      timeOptions,
    ).format(startTime);
    const appointmentEndTimeStr = new Intl.DateTimeFormat(
      'es-CO',
      timeOptions,
    ).format(endTime);
    ...
    const relevantAvailability = availabilities.find((av) => {
      return (
        appointmentStartTimeStr >= av.startTime &&
        appointmentEndTimeStr <= av.endTime
      );
    });

    if (!relevantAvailability) {
      throw new BadRequestException(
        `El empleado no está disponible en este horario (${dayOfWeek} de ${appointmentStartTimeStr} a ${appointmentEndTimeStr}).`,
      );
    }
    ...
    if (appointmentIdToExclude) {
      query = query.andWhere('appointment.id != :appointmentIdToExclude', {
    ...
    if (
      overlappingAppointmentsCount >= relevantAvailability.maxAppointmentsAtOnce
    ) {
      throw new BadRequestException(
        `El empleado ya tiene el máximo de citas (${relevantAvailability.maxAppointmentsAtOnce}) agendadas simultáneamente en este horario.`,
      );
    }
  }
  ...
  async create(
    ...
    if (
      createAppointmentDto.status === AppointmentStatus.REALIZADA &&
      createAppointmentDto.paymentStatus !== PaymentStatus.PAGADO
    ) {
      throw new BadRequestException(
        'El estado de pago debe ser "Pagado" si el estado de la cita es "Realizada".',
      );
    }
    ...
    if (fullAppointment) {
      ...
      if (
        fullAppointment.status === AppointmentStatus.CONFIRMADA &&
        fullAppointment.client?.phone
      ) {
        this.logger.log(
          `Enviando confirmación de cita al cliente: ${fullAppointment.client.phone}`,
        );
  ...
  async update(
    ...
    const newEmployeeId =
      updateAppointmentDto.employeeId ?? currentAppointment.employeeId;
    ...
    const newPaymentStatus =
      updateAppointmentDto.paymentStatus ?? currentAppointment.paymentStatus;

    if (
      newStatus === AppointmentStatus.REALIZADA &&
    ...
    if (fullUpdatedAppointment) {
      ...
      if (
        ...
        currentAppointment.status !== AppointmentStatus.CONFIRMADA &&
      ...
      if (
        newStartTime.getTime() !== currentAppointment.startTime.getTime() ||
        newEndTime.getTime() !== currentAppointment.endTime.getTime()
      ) {
        changes.push(
          `La nueva hora es ${newStartTime.toLocaleString('es-ES', { dateStyle: 'short', timeStyle: 'short' })}`,
        );
      }
      if (newEmployeeId !== currentAppointment.employeeId) {
        changes.push(
          `El nuevo profesional asignado es ${fullUpdatedAppointment.employee.name}`,
        );
      }
      ...
      if (changes.length > 0 && fullUpdatedAppointment.employee?.phone) {
        ...
        if (
          ...
          this.logger.log(
            'Esperando 7 segundos antes de enviar la actualización al empleado...',
          );
          await delay(7000);
        }
