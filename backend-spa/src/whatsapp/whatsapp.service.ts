...
export class WhatsappService {
  ...
  constructor(
    ...
    this.appTimezone =
      this.configService.get<string>('APP_TIMEZONE') || 'America/Bogota';

    if (!this.apiUrl || !this.apiKey) {
      this.logger.error(
        'WASENDER_API_URL o WASENDER_API_KEY no están configuradas en el .env',
      );
      throw new InternalServerErrorException(
        'Configuración de WhatsApp incompleta. Verifique el archivo .env',
      );
    }
  }
  ...
  async sendMessage(to: string, message: string): Promise<any | null> {
    try {
      ...
      this.logger.log(
        `Intentando enviar mensaje a ${to}: ${message.substring(0, 50)}...`,
      );
      const response = await axios.post(this.apiUrl, payload, { headers });
      this.logger.log(
        `Mensaje enviado a ${to}. Respuesta: ${JSON.stringify(response.data)}`,
      );
      ...
      if (axios.isAxiosError(error)) {
        this.logger.error(
          `Respuesta de WasenderAPI (error): ${JSON.stringify(error.response?.data)}`,
        );
      }
   ...
   */
  async sendAppointmentConfirmationToClient(
    clientPhone: string,
    clientName: string,
    employeeName: string,
    serviceName: string,
   ...
   */
  async sendNewAppointmentToEmployee(
   ...
   */
  async sendAppointmentUpdateToEmployee(
    employeePhone: string,
    employeeName: string,
    clientName: string,
    serviceName: string,
   ...
   */
  async sendAppointmentCancellationToEmployee(
    employeePhone: string,
    employeeName: string,
    clientName: string,
    serviceName: string,
    ...
    for (const client of clients) {
      if (client.phone) {
        ...
        if (imageUrl) {
          ...
          result = await this.sendMessageWithImage(
            client.phone,
            imageUrl,
            message,
          );
        ...
        this.logger.warn(
          `Cliente ${client.name} (ID: ${client.id}) no tiene número de teléfono.`,
        );
        failedCount++;
      }
    }
  ...
  async sendAppointmentReminder(
    ...
    const message = `¡Hola ${clientName}! Te recordamos tu cita en Casa Bonita para el servicio de ${serviceName} el ${formattedDate}. ¡Te esperamos!`;
    await this.sendMessage(clientPhone, message);
  }
}
